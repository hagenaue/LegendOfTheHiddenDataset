...
testtypeOfOutput <- c("Tstat", "Beta") #all values must be in one column
...
variableCols <- HiddenAffy(AffyDataModel = AffyDataSetModel, geneList = testgeneList, datasets = testdatasets, brainRegion = testBrainRegion, variablesInterest = testvariablesInterest, typeOfOutput = testtypeOfOutput)
rm(variableCols)
newerOutPutData <- variableCols[,unique(grep(paste(testtypeOfOutput,collapse="|"), colnames(variableCols), ignore.case = TRUE, perl = TRUE, value=F))]
rm(newerOutPutData)
...
HiddenAffy <- function(AffyDataModel, geneList, datasets, brainRegion, variablesInterest, typeOfOutput){
    ...
    newerOutPutData <- variableCols[,unique(grep(paste(typeOfOutput,collapse="|"), colnames(variableCols), ignore.case = TRUE, perl = TRUE, value=F))]
